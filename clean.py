...
for root, dirs, files in os.walk(path):
    for file in files:
        ...
        if re.search('((s|S)(a|A)(m|M)(p|P)(l|L)(e|E))', file):
            delete.append(f)
            break
        elif file.endswith(".avi") or file.endswith('.mp4') or file.endswith('.mkv') or file.endswith('.mpeg4') or file.endswith('.m4v') or file.endswith('.srt') or file.endswith('.sub') or file.endswith('.sbv') or file.endswith('mp3'):
            f = os.path.join(root, file)
            #print (f)
            d = guessit(f)
            #print(d)
